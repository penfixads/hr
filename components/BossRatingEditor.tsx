    ...
    <>
      {overallAvg > 0 && (
        <div className="mb-6 flex justify-end">
          <div className="flex items-center gap-3 bg-penfix-card border rounded-xl px-5 py-3 shadow-sm">
            <div>
              <div className="text-xs text-penfix-text-muted">Overall Score</div>
              <div className="text-2xl font-bold" style={{ color: '#D9BB6E' }}>{overallAvg.toFixed(2)}</div>
            </div>
            ...
            {score.bonusRatedCount > 0 && (
              <div className="border-l pl-3">
                <div className="text-xs text-penfix-text-muted">Core</div>
                <div className="font-bold text-sm" style={{ color: '#D9BB6E' }}>{score.core.toFixed(2)}</div>
                <div className="text-xs text-penfix-text-muted">
                  + {score.uplift.toFixed(2)} bonus
                </div>
              </div>
            )}
            <div className="border-l pl-3">
              <div className="text-xs text-penfix-text-muted">December Raise</div>
              <div className="font-bold text-sm" style={{ color: raise.color }}>{raise.label}</div>
              <div className="text-xs text-penfix-text-muted">{raise.note}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-penfix-card rounded-xl border shadow-sm p-6 mb-6">
        <div className="flex items-center justify-between mb-4 pb-2 border-b">
          <h3 className="font-bold text-base" style={{ color: '#D9BB6E' }}>Skills Assessment</h3>
          <div className="flex gap-4 text-xs text-penfix-text-muted">
            <span className="flex items-center gap-1"><span style={{ color: '#C9A84C' }}>★</span> Self</span>
            <span className="flex items-center gap-1"><span style={{ color: '#D9BB6E' }}>★</span> Management</span>
          ...
          return (
            <div key={category} className="mb-8">
              <div className={`flex items-center justify-between ${isBonus ? 'mb-1' : 'mb-3'}`}>
                <h4 className="font-semibold text-sm flex items-center gap-2" style={{ color: '#D9BB6E' }}>
                  {category}
                  {isBonus && (
                ...
                {catAvg > 0 && (
                  ...
                  <span className="text-sm font-bold" style={{ color: isBonus ? '#9CA3AF' : '#D9BB6E' }}>
                    {isBonus ? 'Bonus Avg' : 'Category Avg'}: {catAvg.toFixed(1)}
                  </span>
                )}
              </div>
              {isBonus && <p className="text-xs text-penfix-text-muted mb-3">{BONUS_CATEGORY_NOTE}</p>}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {catSkills.map((skill) => {
                      ...
                      return (
                        <tr key={skill} className="border-b border-penfix-border hover:bg-penfix-surface-muted">
                          <td className="py-3 pr-4 text-foreground">{skill}</td>
                              ...
                              <div className="flex gap-0.5">
                                {[1, 2, 3, 4, 5].map(star => (
                                  <button key={star} type="button"
                                    ...
                                    <span style={{ color: star <= b ? '#D9BB6E' : 'rgba(255,255,255,0.2)' }}>★</span>
                                  </button>
                                ))}
                              </div>
                            </div>
                          ...
                          <td className="py-3 px-3 text-center">
                            {avg > 0 ? (
                              <span className="font-bold" style={{ color: '#D9BB6E' }}>{avg.toFixed(1)}</span>
