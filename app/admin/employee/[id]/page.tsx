...
export default async function EmployeeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!employee) {
    return (
      <div className="flex flex-col min-h-screen">
        <PenfixHeader />
        <main className="flex-1 flex items-center justify-center text-penfix-text-muted">Employee not found.</main>
        <PenfixFooter />
      </div>
    )
  }
  ...
  const infoRow = (label: string, value?: string) => value ? (
    <div key={label} className="flex gap-2 text-sm py-1.5 border-b border-penfix-border">
      <span className="text-penfix-text-muted w-48 shrink-0">{label}</span>
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 px-4 py-8 max-w-4xl mx-auto w-full">
        ...
        {/* Emergency Contact */}
        <div className="bg-penfix-card rounded-xl border shadow-sm p-6 mb-6">
          <h3 className="font-bold text-base mb-4 pb-2 border-b" style={{ color: '#D9BB6E' }}>Emergency Contact</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
