...
export default function RubricForm({
  ...
  return (
    <div className="bg-penfix-card rounded-xl border shadow-sm p-6 mb-5">
      <h3 className="font-bold text-base mb-1 pb-2 border-b" style={{ color: MAROON_TEXT }}>Reviewer rubric</h3>
      <p className="text-xs text-penfix-text-muted mt-2 mb-4">
      ...
      <div className="flex flex-col gap-3">
        {RUBRIC_CRITERIA.map(c => (
          <div key={c} className="flex items-center gap-3 flex-wrap">
            <span className="text-sm text-foreground w-40 shrink-0">{c}</span>
            <div className="flex gap-1.5">
              {[1, 2, 3, 4, 5].map(n => (
                <button key={n} type="button" onClick={() => setRubric(prev => ({ ...prev, [c]: n }))}
                  aria-label={`${c}: ${n} of 5`}
                  className={`w-9 h-9 rounded-lg border text-sm font-semibold ${
                    rubric[c] === n ? 'text-white' : 'text-penfix-text-muted bg-penfix-card hover:bg-amber-50'
                  }`}
      ...
      <label className="flex flex-col gap-1 mt-5">
        ...
        <textarea rows={3} value={notes} onChange={e => setNotes(e.target.value)}
          ...
          className="border border-penfix-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-200" />
      </label>
      ...
      <div className="flex items-center gap-3 mt-4 flex-wrap">
        ...
        {reviewedBy && (
          <span className="text-xs text-penfix-text-muted ml-auto">Last reviewed by {reviewedBy} · {reviewedAt}</span>
        )}
      </div>
    </div>
  )
